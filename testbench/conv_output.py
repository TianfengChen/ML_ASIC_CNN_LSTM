...
input_arrays=[]
print_input_arrays=[]
for i in range(0,input_array_num):#range(0,4)
...
for j in range(0,weight_array_num*input_array_num):
        ...
        weight_array=np.round(weight_array)#
...
def conv_one_input_one_weight(data,weight,stride):
        ...
        for idx_r, r in enumerate(range(row_start, row_end,stride)):
                for idx_c, c in enumerate(range(col_start, col_end,stride)):
                        region=data[r:r+weight.shape[0],c:c+weight.shape[1]]#input_array -> data
...
def conv(input_arrays,weight_arrays,stride):#need conv_one_input_one_weight()
        conv_result=[]
        for x in range(0,weight_array_num):#range(0,4)
                one_weight_result=[]
                for y in range(0,input_array_num):#range(0,4)
                        one_weight_result.append(conv_one_input_one_weight(input_arrays[y],weight_arrays[x*input_array_num+y],stride))
                conv_int = sum(one_weight_result)
                row_start, row_end = 0, conv_int.shape[0]
                col_start, col_end = 0, conv_int.shape[1]
                for r in range(row_start, row_end):
                        for c in range(col_start, col_end):
                                conv_int[r,c]=conv_int[r,c]%256
                                if conv_int[r,c] >= 128:
                                        conv_int[r,c]=conv_int[r,c]-256
                conv_result.append(conv_int)
        return conv_result
...
def max_pooling(data,filter_size):
        ...
        for idx_r, r in enumerate(range(row_start, row_end,stride)):
                for idx_c, c in enumerate(range(col_start, col_end,stride)):
                        region=data[r:r+filter_size,c:c+filter_size]#input_array -> data
